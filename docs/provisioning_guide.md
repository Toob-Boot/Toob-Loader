...
## 2. Toobfuzzer Kalibrierung
Bevor das Bootloader-Binary final kompiliert wird, durchläuft die Zielhardware die `toobfuzzer` Pipeline:
- **(GAP-F30)** Der Fuzzer ermittelt die **exakte** `sector_size`, `max_erase_time_us`, `write_alignment` und kritischen ROM-Punkte für Krypto/Watchdog.
- Generierung von **zwei** Artefakten: `aggregated_scan.json` (Hardware-Map) und `blueprint.json` (Logik/ROM-Limits).
- **(GAP-F10)** Der VDD-Drop (Brownout-Penalties) für das `min_battery_mv` Limit wird manuell vom Entwickler im `device.toml` kalibriert (kein Fuzzer-Output).
- **(GAP-F16)** Die Brute-Force-Fuzzer-Scans strapazieren das Flash. Das Tool exportiert seinen eigenen Sektor-Verschleiß als `fuzzer_erase_overhead`, welcher als Startwert (`wal_erase_count` > 0) verwendet werden MUSS, um korrekte Telemetrie/Lebensdauer-Berechnungen durch das C-Binary sicherzustellen.

## 3. Kompilierung (Manifest Compiler)
**(GAP-F30)** Das `aggregated_scan.json` UND das `blueprint.json` File werden in den Manifest-Compiler geworfen. 
Dieser webt die Limits als C-Makros (`#define`) sicher in die Datei `chip_config.h` (Bootloader) und `libtoob_config.h` (OS) ein. 
