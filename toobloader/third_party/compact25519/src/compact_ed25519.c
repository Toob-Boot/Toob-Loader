 ...
 * tag_status: auto
 */
void compact_ed25519_keygen(uint8_t private_key[ED25519_PRIVATE_KEY_SIZE],
  ...
  memcpy(private_key + __PUBLIC_KEY_OFFSET, public_key,
...
void compact_ed25519_calc_public_key(
  ...
  memcpy(public_key, private_key + __PUBLIC_KEY_OFFSET,
         ED25519_PUBLIC_KEY_SIZE);
}

void compact_ed25519_sign(uint8_t signature[ED25519_SIGNATURE_SIZE],
                          const uint8_t private_key[ED25519_PRIVATE_KEY_SIZE],
                          const void *message, size_t msg_length) {
  edsign_sign(signature, private_key + __PUBLIC_KEY_OFFSET, private_key,
              message, msg_length);
}

bool compact_ed25519_verify(const uint8_t signature[ED25519_SIGNATURE_SIZE],
