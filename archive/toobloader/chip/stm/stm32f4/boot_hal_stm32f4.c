/**
 * @file boot_hal_stm32f4.c
 * @brief Bootloader HAL Shim — STM32F4 Specific
 *
...
boot_flash_write_raw(uint32_t addr, const uint8_t *data, uint32_t len) {
  (void)addr;
  (void)data;
  (void)len;
  return 0;
...
void boot_uart_puts(const char *msg) { (void)msg; }

KB_FEATURE("boot_hal_stm32f4")
