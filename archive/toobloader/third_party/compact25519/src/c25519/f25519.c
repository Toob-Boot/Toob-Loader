...
void f25519_normalize(uint8_t *x) {
  ...
  (void)f25519_select(x, minusp, x, (c >> 15) & 1);
}
 ...
 */
KB_CORE()
void f25519_select(uint8_t *dst, const uint8_t *zero, const uint8_t *one,
                   uint8_t condition) {
...
void f25519_neg(uint8_t *r, const uint8_t *a) {
  RP_ASSERT(r != NULL, "fneg r");
  RP_ASSERT(a != NULL, "fneg a");
  uint32_t c = 0;
  int i;
...
void f25519_mul__distinct(uint8_t *r, const uint8_t *a, const uint8_t *b) {
  RP_ASSERT(r != NULL, "fmul_d r");
  RP_ASSERT(a != NULL, "fmul_d a");
  RP_ASSERT(b != NULL, "fmul_d b");
  uint32_t c = 0;
  int i;
...
void f25519_mul(uint8_t *r, const uint8_t *a, const uint8_t *b) {
  ...
  (void)f25519_mul__distinct(tmp, a, b);
  (void)f25519_copy(r, tmp);
}
#endif

void f25519_mul_c(uint8_t *r, const uint8_t *a, uint32_t b) {
  RP_ASSERT(r != NULL, "fmul_c r");
  RP_ASSERT(a != NULL, "fmul_c a");
  uint32_t c = 0;
  int i;
...
void f25519_inv__distinct(uint8_t *r, const uint8_t *x) {
  RP_ASSERT(r != NULL, "finv_d r");
  RP_ASSERT(x != NULL, "finv_d a");
...
void f25519_inv(uint8_t *r, const uint8_t *x) {
  ...
  RP_ASSERT(x != NULL, "finv x");
  uint8_t tmp[F25519_SIZE];

  (void)f25519_inv__distinct(tmp, x);
  (void)f25519_copy(r, tmp);
}
#endif
...
static void exp2523(uint8_t *r, const uint8_t *x, uint8_t *s) {
  RP_ASSERT(r != NULL, "exp r");
  RP_ASSERT(x != NULL, "exp x");
  RP_ASSERT(s != NULL, "exp s");
  int i;
  ...
  /* 1 1 */
  (void)f25519_mul__distinct(r, x, x);
  (void)f25519_mul__distinct(s, r, x);

  /* 1 x 248 */
  for (i = 0; i < 248; i++) {
    (void)f25519_mul__distinct(r, s, s);
    (void)f25519_mul__distinct(s, r, x);
  }
  ...
  /* 1 */
  (void)f25519_mul__distinct(s, r, r);
  (void)f25519_mul__distinct(r, s, x);
}

void f25519_sqrt(uint8_t *r, const uint8_t *a) {
  RP_ASSERT(r != NULL, "fsqrt r");
  RP_ASSERT(a != NULL, "fsqrt a");
  ...
  /* v = (2a)^((p-5)/8) [x = 2a] */
  (void)f25519_mul_c(x, a, 2);
  (void)exp2523(v, x, y);

  /* i = 2av^2 - 1 */
  (void)f25519_mul__distinct(y, v, v);
  (void)f25519_mul__distinct(i, x, y);
  (void)f25519_load(y, 1);
  (void)f25519_sub(i, i, y);

  /* r = avi */
  (void)f25519_mul__distinct(x, v, a);
  (void)f25519_mul__distinct(r, x, i);
}
