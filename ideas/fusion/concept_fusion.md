...
- **Stage 1 (Updateable Engine):** Das dynamische Herz (Minimalversion ~14 KB, Full-Feature ~24-28 KB). Trägt alle Module: Update-Engine (WAL + Merkle), Crypto-HAL (Pluggable: Micro-Crypto wie `monocypher`, `chacha20`, `compact25519` oder direkte Hardware-Crypto-Wrapper statt des viel zu schweren mbedTLS!), SUIT-Parser, DICE-Identity (Achtung: Ein Update von Stage 1 ändert CDI-Identität. Um Flotten-Lockouts zu verhindern, emittiert Stage 1 vor dem Reboot ein **DICE_MIGRATION_EVENT** mit einem von der alten Identity signierten "Transition-Token". Das Backend validiert diesen kryptografischen Beweis und hasht die neue Identität nahtlos, ohne manuelles 10.000er Flotten-Re-Provisioning!), Delta-Patcher, Flash-HAL, Diagnostics, Energy-Guard.
...
**Preflight-Automatisierung:** Der Compiler berechnet aus TOML-Werten (`merkle.chunk_size`) automatisch den exakten RAM-Bedarf der Sibling-Hashes und ermittelt den **minimalen Watchdog-Timeout** (`flash_erase_time_per_sector * sectors`), um zu kurze WDT-Bricks beim Erase physikalisch auszuschließen.
