...
- **Stage 1 (Updateable Engine):** Das dynamische Herz (Minimalversion ~14 KB, Full-Feature ~24-28 KB). Trägt alle Module: Update-Engine (WAL + Merkle), Crypto-HAL (Pluggable: Micro-Crypto wie `monocypher`, `chacha20`, `compact25519` oder direkte Hardware-Crypto-Wrapper statt des viel zu schweren mbedTLS!), SUIT-Parser, DICE-Identity (Achtung: Ein Update von Stage 1 ändert den hardware-basierten Hash und somit die CDI-Identität! Dies bedingt zwingend einen Remote-Attestation / Re-Provisioning Workflow im IoT-Backend vor dem Reboot!), Delta-Patcher, Flash-HAL, Diagnostics, Energy-Guard.

*Das zu startende OS (Application / RTOS) bestätigt den erfolgreichen Start final via eines Confirm-Flags.* Damit das OS völlig vom Bootloader-Flash entkoppelt bleibt, schreibt das OS ein flüchtiges Magic-Byte ins `uninitialisierte RTC-RAM` und führt einen **Soft-Reset** aus. Der Bootloader (Stage 1) erwacht, findet das Flag im RAM, sichert den permanenten State in den Flash-WAL-Speicher, löscht das RAM und bootet das OS endgültig (= keine Kaltstart/Zieh-Stecker-Fallen; keine Verletzung der App-Image-Signatur durch modifizierte Flash-Sektor-Bytes). Bleibt dies aus (Watchdog), leitet Stage 1 beim nächsten Start einen Rollback ein.
...
- [x] **`crypto_hal_t`** (hash · verify · rng) -> PFLICHT
- [x] **`clock_hal_t`** (init · get_tick · delay) -> PFLICHT
- [x] **`wdt_hal_t`** (kick · set_timeout) -> **PFLICHT für Auto-Rollback** (Ohne Hardware-Watchdog kann ein gebricktes OS nie resetten. Fallback ist ein rein manueller OS-Testmodus).
- [ ] **`console_hal_t`** (putchar · getchar) -> OPTIONAL (Serieller Log)
- [ ] **`power_hal_t`** (battery_level · sleep) -> OPTIONAL (Energy-Guard: Misst Batterie-Spannung zwingend **unter Last (Dummy-Load/CPU-Spike)**. Reine Leerlaufmessungen an LiPos sind nutzlos und führen unweigerlich zum Brownout bei plötzlichem Flash-Erase!)
...
- **Das Write-Ahead Log (WAL):** Ein winziges (~128 Bytes) Journal auf dem Flash. Damit Flash-Speicher nicht durch den ewigen 128-Byte-Verschleiß getötet wird, operiert das WAL als **Append-Only Ring Buffer über mindestens 2 pre-erased Flash-Sektoren**. NOR-Flash erlaubt das sequenzielle Anhängen via "Bits auf 0 setzen" ohne teure Block-Erases (Vermeidung extremer Schreib-Amplifikation). Zum Schutz vor kritischen Stromausfällen nutzt die WAL-Garbage Collection asymmetrisches *Pre-Erasing*: Der alte Sektor wird erst asynchron/während Idle-Zeiten nach dem ersten erfolgreichen `TXN_COMMIT` des neuen Sektors gelöscht.
...
_Sonderfaktor Delta-Updates:_ Da Staging winzig sein kann, ermöglicht der Delta-Patcher eine solide, **realistische 3- bis 5-fache Reduktion**. (Das oft beworbene "18x" erfordert Dekomprimierung via gigantischem RAM-Sliding-Window, was beim strikten `14 KB` RAM Limit der Stage 1 verheerende Stack-Overflows reißt. Es kommen daher nur dedizierte Micro-Dictionary Patcher wie `heatshrink` infrage). Zwingende Architektur-Bedingung: Dieser Patcher muss **strictly Forward-Only und Non-Overlapping** abarbeiten, um alte Lese-Referenzen nicht mitten im In-Place Swap unwiederbringlich zu überschreiben!
...
- **Host-Nativer Sandbox-HAL:** Der gesamte Bootloader kann auf dem PC (macOS/Linux) als natives Binary kompiliert werden (`$ boot-build --sandbox`). Der Flash-Speicher wird durch eine simple Text-Datei abgebildet.
