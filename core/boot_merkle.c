 ...
 * 2. Glitch-Resistant Tearing Defense: Forward/Backward Hash-Comparison blockiert
 *    Single-Bit Voltage-Glitches in der Krypto-Evaluierung komplett.
 * 3. Array Overflow Traps (CVE-Prevention): Sichert (num_chunks * HASH_LEN) und
 *    Flash-Pointer auf 32-bit Architekturen absolut gegen Integer Wraparounds ab.
 * 4. P10 Stack Zeroization: Nullifiziert Residuen zwingend nach Nutzung via Single-Exit.
 */
...
_Static_assert(BOOT_OK == 0x55AA55AA, "BOOT_OK muss zwingend ein High-Hamming-Weight Pattern sein, um das Double-Check Pattern zu garantieren!");

/**
 * @brief Führt einen speichersicheren, konstanten Zeit-Vergleich für SHA-256 Hashes aus.
 * O(1) Laufzeit ohne SRAM-Bus-Stalls, doppelt gesichert gegen physikalische Voltage-Glitches.
 */
static inline boot_status_t constant_time_memcmp_32_glitch_safe(const uint8_t *a, const uint8_t *b) {
    uint32_t acc_fwd = 0;
    uint32_t acc_rev = 0;

    /* ALU Loop Unrolling durch den Compiler erlauben, aber Datenabhängigkeit erzwingen.
     * Vorwärts- und Rückwärts-Evaluierung macht Timing-Orakel und Fault-Injection extrem schwer.
     * Byte-Zugriffe gewährleisten Cortex-M0 (Unaligned-Access) Kompatibilität ohne HardFaults. */
    for (uint32_t i = 0; i < BOOT_MERKLE_HASH_LEN; i++) {
        acc_fwd |= (uint32_t)(a[i] ^ b[i]);
        acc_rev |= (uint32_t)(a[BOOT_MERKLE_HASH_LEN - 1 - i] ^ b[BOOT_MERKLE_HASH_LEN - 1 - i]);
    }
...
boot_status_t boot_merkle_verify_stream(const boot_platform_t* platform,
    ...
    uint64_t hash_ctx[BOOT_MERKLE_MAX_CTX_SIZE / sizeof(uint64_t)] __attribute__((aligned(8)));
    ...
    /* 3.1 Wrap-Check: Verhindert (image_flash_addr + image_size) Overflow in der Flash-Zone */
    if (UINT32_MAX - image_flash_addr < image_size) {
        stat = BOOT_ERR_FLASH_BOUNDS;
        goto cleanup;
    }

    /* 3.2 Wrap-Check: Verhindert Chunk-Multiplikation Overflows bei präparierten Arrays */
    if (num_chunks > (UINT32_MAX / chunk_size)) {
        stat = BOOT_ERR_INVALID_ARG;
        goto cleanup;
    }
    
    /* 3.3 Array Overflow Trap: Sichert (num_chunks * HASH_LEN) auf 32-bit Architekturen absolut ab */
    if (num_chunks > (UINT32_MAX / BOOT_MERKLE_HASH_LEN)) {
        stat = BOOT_ERR_INVALID_ARG;
        goto cleanup;
    }
    if ((num_chunks * BOOT_MERKLE_HASH_LEN) > chunk_hashes_len) {
        stat = BOOT_ERR_INVALID_ARG; /* Out-Of-Bounds Buffer Leak Versuch unwiderruflich blockiert */
        goto cleanup;
    }

    /* 3.4 Pointer-Space Wrap-Around Guard */
    if ((uintptr_t)chunk_hashes > (UINTPTR_MAX - (num_chunks * BOOT_MERKLE_HASH_LEN))) {
        stat = BOOT_ERR_INVALID_ARG;
        goto cleanup;
    }
    ...
    if (platform->flash->write_align > 0 && (chunk_size % platform->flash->write_align) != 0) {
        stat = BOOT_ERR_INVALID_ARG;
        goto cleanup;
    }
    ...
    /* 3.7 Malleability by Truncation Defense (Exakte Hash-Anzahl Evaluierung, Wraparound-Safe) 
     * Verhindert Angriffe durch Abschneiden des Manifests. O(1) Branchless Math. */
    uint32_t required_chunks = (image_size / chunk_size) + ((image_size % chunk_size) > 0 ? 1U : 0U);
    ...
    /* O(1) Pointer-Tracker (Reduziert Rechenoverhead / MUL-Instruktionen in der heißen Iteration) */
    ...
    for (uint32_t i = 0; i < num_chunks; i++) {
        /* Anti-Lockup (concept_fusion.md): Watchdog vor schweren IO-Operationen füttern */
        platform->wdt->kick();
        
        uint32_t read_len = (remaining_bytes < chunk_size) ? remaining_bytes : chunk_size;
        ...
        if (stat != BOOT_OK) goto cleanup;

        stat = platform->crypto->hash_update(hash_ctx, crypto_arena, read_len);
        if (stat != BOOT_OK) goto cleanup;

        /* P10 Rule: Buffer sicher nullen, bevor er an externe HW-Register übergeben wird */
        boot_secure_zeroize(computed_hash, sizeof(computed_hash));
        ...
        /* Glitch-Resistant CONSTANT-TIME Verifikation (gibt sauber BOOT_ERR_VERIFY bei Fail zurück) */
        stat = constant_time_memcmp_32_glitch_safe(computed_hash, current_hash_ptr);
        
        /* O(1) Säuberung transienter Secrets sofort nach der Nutzung (Verhindert Leakage bei Interrupts) */
        boot_secure_zeroize(computed_hash, sizeof(computed_hash));
    ...
    /* Mathematischer Coverage-Beweis gegen Loop-Abortion Glitches (Voltage Faults)
     * Verhindert Angriffe, bei denen die For-Schleife nach wenigen validen Chunks 
     * gewaltsam zum Abbruch gezwungen wird. */
